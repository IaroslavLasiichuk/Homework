...
function f8() {
  ...
  out8.innerHTML += '<div class="js-2">new div</div>';
}
...
function f13() {
  ...
  const inputValue = document.querySelector('.i-13').value;
  out.textContent = inputValue;
...
function f17() {
  ...
  out.textContent = document.querySelector('.s-17').value;;
}
...
function f18() {
  const select = document.querySelector('.s-18').value;
...
//Цю зроби так шоб в незалежності на який клікнути міняв обидва!


function f20() {
  const select = document.querySelector('.s-201').value;
  const select1 = document.querySelector('.s-202');
